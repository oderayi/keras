# Convolutional neural network on CIFAR 10 with Residual Net (ResNet) module
...
def ResNet(x, filters, pool=False):
    res = x
    if pool:
...
def MiniModel(input_shape):
    ...
    net = ResNet(net, 64, pool=True)
    net = ResNet(net, 64)
    net = ResNet(net, 64)

    net = ResNet(net, 128, pool=True)
    net = ResNet(net, 128)
    net = ResNet(net, 128)

    net = ResNet(net, 256, pool=True)
    net = ResNet(net, 256)
    net = ResNet(net, 256)
